...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'generate') {
    ...
    // Guardar en /public/catalogos/
    $abs          = dirname(__DIR__) . '/catalogos/' . $file;

    /* =============== PLANTILLA CRUD COMPLETA ================== */
    $tpl = <<<'PHP'
<?php
require_once __DIR__ . '/../../app/db.php';
if (session_status() === PHP_SESSION_NONE) { //@session_start(); }
...
PHP;
    ...
    // Redirección absoluta correcta a /public/catalogos/
    $baseName = basename($abs);
    $url = '/assistpro_kardex_fc/public/catalogos/'.$baseName;

    echo "<script>
        (function(){
            var url = '$url';
            if (parent && parent !== window && parent.document && parent.document.getElementById('content')) {
