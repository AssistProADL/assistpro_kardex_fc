...
function validar($data)
{
  ...
  if ($cve === '')
    $errs[] = 'cve_proveedor es obligatorio';
  if ($pais === '')
    $errs[] = 'pais es obligatorio';
  if ($emp === '' && $nom === '')
    $errs[] = 'Debe capturar Empresa o Nombre';
  return $errs;
}
...
if ($action === 'list') {
  ...
  if ($empresa !== '')
    $stc->bindValue(':empresa', $empresa, PDO::PARAM_STR);
  if ($q !== '')
  ...
  $sql = "SELECT
      ...
      direccion,colonia,telefono2,ID_Externo,cve_dane,latitud,longitud,es_transportista
    FROM c_proveedores
    $where
  ...
  if ($empresa !== '')
    $st->bindValue(':empresa', $empresa, PDO::PARAM_STR);
  if ($q !== '')
...
switch ($action) {
  ...
  case 'create': {
    ...
    $st = $pdo->prepare("
      INSERT INTO c_proveedores
      (cve_proveedor,Empresa,Nombre,RUT,direccion,colonia,ciudad,estado,pais,telefono1,telefono2,ID_Externo,cve_dane,latitud,longitud,Activo,es_cliente,es_transportista,envio_correo_automatico)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ");
    $st->execute([
      ...
      i1($_POST['Activo'] ?? 1),
      i1($_POST['es_cliente'] ?? 0),
      i1($_POST['es_transportista'] ?? 0),
      i1($_POST['envio_correo_automatico'] ?? 0),
    ]);
  ...
  case 'update': {
    ...
    if (!$id) {
      echo json_encode(['error' => 'ID_Proveedor requerido']);
      exit;
    }
    $errs = validar($_POST);
    if ($errs) {
    ...
    $st = $pdo->prepare("
      UPDATE c_proveedores SET
        ...
        es_cliente=?, es_transportista=?, envio_correo_automatico=?
