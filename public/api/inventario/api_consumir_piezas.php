...
function load_pdo() {
  $candidates = [
    __DIR__ . '/../../app/db.php',           // /public/api/inventario -> /public/app/db.php
    __DIR__ . '/../../../app/db.php',
    __DIR__ . '/../../../../app/db.php',
  ];
...
if ($bl_origen === '' || $articulo === '' || $cantidad <= 0) j_err("Datos incompletos (bl_origen, articulo, cantidad)");

$table = 'ts_existenciapiezas';
if (!table_has($pdo, $table)) j_err("No existe tabla $table", 500);

$C = cols($pdo, $table);
$colBL   = pick_col($C, ['bl','BL','ubicacion','Cve_Ubicacion','cve_ubicacion','Origen','origen']);
...
$where = ["`$colBL`=?","`$colArt`=?"];
$args  = [$bl_origen, $articulo];

if ($colLP) {
  if ($lp_origen !== '') { $where[]="`$colLP`=?"; $args[]=$lp_origen; }
}
...
try {
  ...
  if (!$row) throw new Exception("No hay existencia para consumir (piezas).");
