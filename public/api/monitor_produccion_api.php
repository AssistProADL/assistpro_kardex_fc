 ...
 * Soporta:
 * action=stats   -> KPIs
 * action=detalle -> header + lines (td_ordenprod)
 */
...
if ($action === 'detalle') {
  ...
  try {
    ...
    // detalle componentes (PRIORIDAD: td_ordenprod)
    $lines = [];
    $detTables = ['td_ordenprod','t_ordenprod_det','t_ordenprod_detalle','t_ordenprod_d'];
    $detTable = null;
    ...
    if ($detTable) {
      ...
      $cFol = colExists($pdo,$detTable,'Folio_Pro') ? 'Folio_Pro' : (colExists($pdo,$detTable,'folio') ? 'folio' : (colExists($pdo,$detTable,'Folio') ? 'Folio' : null));
