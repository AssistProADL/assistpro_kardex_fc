...
try {
  ...
  $sql = "
    ...
    WHERE Activo = 1
      ...
      AND CodigoCSD <> ''
      AND AcomodoMixto = 'S'
      AND (:almacen1 = '' OR cve_almac = :almacen2)
      AND (:q1 = '' OR CodigoCSD LIKE CONCAT('%', :q2, '%'))
    ORDER BY CodigoCSD
    LIMIT :limit
  ";

  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(':almacen1', $almacen);
  $stmt->bindValue(':almacen2', $almacen);
  $stmt->bindValue(':q1', $q);
  $stmt->bindValue(':q2', $q);
  ...
  respond(['ok' => 0, 'error' => $e->getMessage() . ' [ubicaciones_destino]'], 500);
}
