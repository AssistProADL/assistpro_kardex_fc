...
function norm_bool01($v, $def = 0): int
{
  ...
  if (in_array($s, ['1', 'SI', 'S', 'TRUE', 'T', 'Y', 'YES'], true))
    return 1;
  if (in_array($s, ['0', 'NO', 'N', 'FALSE', 'F'], true))
    return 0;
  return (int) $def;
}
...
try {
  ...
  if ($action === 'list') {
    ...
    if ($q !== '') {
      $where[] = '(cve_ruta LIKE ? OR descripcion LIKE ? OR CAST(cve_almacenp AS CHAR) LIKE ? OR CAST(ID_Ruta AS CHAR) LIKE ?)';
      $qp = '%' . $q . '%';
    ...
    // KPIs globales (sin filtros de búsqueda ni inactivos) - Queremos totales reales
    $kpi_sql = '';
  ...
  if ($action === 'almacenes') {
    $rows = db_all('SELECT id, nombre FROM c_almacenp WHERE IFNULL(Activo,1)=1 ORDER BY nombre');
  ...
  if ($action === 'get') {
    ...
    if ($id <= 0)
      jexit(false, ['message' => 'ID inválido'], 400);
    ...
    if (!$row)
      jexit(false, ['message' => 'No encontrado'], 404);
  ...
  if ($action === 'recover') {
    ...
    // Reactivar
    dbq('UPDATE t_ruta SET Activo=1 WHERE ID_Ruta=?', [$id]);
    jexit(true, ['message' => 'OK']);
  }
  ...
  if ($action === 'import_csv') {
    ...
    $fh = fopen($tmp, 'r');
    if (!$fh)
      jexit(false, ['message' => 'No se pudo leer el archivo'], 400);

    $header = fgetcsv($fh);
    if (!$header)
      jexit(false, ['message' => 'CSV vacío'], 400);
