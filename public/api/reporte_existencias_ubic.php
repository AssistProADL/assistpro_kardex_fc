...
try {
  ...
  if ($f['almacen'] !== '') {
    $where[] = "u.almacen_id = :almacen";
    $params[':almacen'] = $f['almacen'];
  }
  if ($f['zona'] !== '') {
    $where[] = "u.zona_id    = :zona";
    $params[':zona'] = $f['zona'];
  }
  if ($f['bl'] !== '') {
    $where[] = "u.codigocsd  = :bl";
    $params[':bl'] = $f['bl'];
  }

  if ($f['tipo_bl'] !== '') {
    $where[] = "COALESCE(u.tipo_bl,'')   = :tipo_bl";
    $params[':tipo_bl'] = $f['tipo_bl'];
  }
  if ($f['status'] !== '') {
    $where[] = "COALESCE(u.status_bl,'') = :status";
    $params[':status'] = $f['status'];
  }
  if ($f['pasillo'] !== '') {
    $where[] = "COALESCE(u.pasillo,'')   = :pasillo";
    $params[':pasillo'] = $f['pasillo'];
  }
  if ($f['rack'] !== '') {
    $where[] = "COALESCE(u.rack,'')      = :rack";
    $params[':rack'] = $f['rack'];
  }
  if ($f['nivel'] !== '') {
    $where[] = "COALESCE(u.nivel,'')     = :nivel";
  ...
  if ($f['lp_tipo'] !== '') {
    $where[] = "COALESCE(su.lp_tipo,'') = :lp_tipo";
    $params[':lp_tipo'] = $f['lp_tipo'];
  }
  if ($f['lote'] !== '') {
    $where[] = "su.lote = :lote";
    $params[':lote'] = $f['lote'];
