...
function validar($data){
  ...
  if(!$tipo) $errs[]='Tipo_Cat es obligatorio (Entrada/Salida)';
...
if($action==='export_csv'){
  ...
  if($tipo==='datos'){
    $sql = "SELECT ".implode(',',$headers)." FROM c_motivo ORDER BY id";
...
/* =========================
   ...
   *No hay Clave única, así que usamos llave de negocio.
========================= */
if($action==='import_csv'){
  ...
  try{
    ...
    while(($r=fgetcsv($fh))!==false){
      ...
      // normalización tipo: aceptamos E/S o Q/S
      $tipoCat = tipo_guardar($data['Tipo_Cat'] ?? '');
      $data['Tipo_Cat'] = $tipoCat ?? '';
...
if($action==='list'){
  ...
  if($tipo==='Q' || $tipo==='S') $stc->bindValue(':tipo', $tipo, PDO::PARAM_STR);
  ...
  if($tipo==='Q' || $tipo==='S') $st->bindValue(':tipo', $tipo, PDO::PARAM_STR);
