...
function validar_obligatorios($data){
  ...
  if($ub==='')  $errs[]='cve_ubicacion es obligatorio';
  return $errs;
}
...
if($action==='export_csv'){
  ...
  $headers = [
    'ID_Embarque','cve_ubicacion','cve_almac','status','Activo','descripcion','AreaStagging','largo','ancho','alto'
  ];
  fputcsv($out,$headers);
...
if($action==='list'){
  ...
  if($alm>0) $where .= " AND e.cve_almac=:alm";
  if($q!==''){
    $where .= " AND (
...
switch($action){
  ...
  case 'get':{
    ...
    if(!$id){ echo json_encode(['error'=>'ID_Embarque requerido']); exit; }
