...
function validar($data){
  ...
  if($motivo==='') $errs[]='Motivo es obligatorio';
  return $errs;
}
...
switch($action){
  ...
  case 'create':{
    ...
    $st=$pdo->prepare("INSERT INTO MotivosNoVenta (Clave,Motivo,Status) VALUES (?,?,?)");
    $st->execute([
  ...
  case 'update':{
    ...
    if(!$id){ echo json_encode(['error'=>'IdMot requerido']); exit; }
    $errs = validar($_POST);
    if($errs){ echo json_encode(['error'=>'Validación','detalles'=>$errs]); exit; }

    $st=$pdo->prepare("UPDATE MotivosNoVenta SET Clave=?, Motivo=?, Status=? WHERE IdMot=? LIMIT 1");
    $st->execute([
      trim((string)$_POST['Clave']),
      trim((string)$_POST['Motivo']),
      i1($_POST['Status'] ?? 1),
  ...
  case 'delete':{
    $id = (int)($_POST['id'] ?? 0);
    if(!$id){ echo json_encode(['error'=>'id requerido']); exit; }
    $pdo->prepare("UPDATE MotivosNoVenta SET Status=0 WHERE IdMot=?")->execute([$id]);
    echo json_encode(['success'=>true]); exit;
  }
  case 'restore':{
    $id = (int)($_POST['id'] ?? 0);
    if(!$id){ echo json_encode(['error'=>'id requerido']); exit; }
    $pdo->prepare("UPDATE MotivosNoVenta SET Status=1 WHERE IdMot=?")->execute([$id]);
    echo json_encode(['success'=>true]); exit;
  }
  default:
