...
function acl_where_emp_sql($alias = 'v') {
  ...
  if (!$ids || !is_array($ids)) return "1=0";
...
function dbq_quote($s){ return "'".str_replace("'", "''", $s)."'"; }
...
if ($almacen   !== '') { $where[] = "TRIM(v.cve_almac)     = TRIM(:almacen)";   $params[':almacen']   = $almacen; }
if ($usuario   !== '') { $where[] = "TRIM(v.id_usuario)    = TRIM(:usuario)";   $params[':usuario']   = $usuario; }
if ($tipo      !== '') { $where[] = "v.tipo_mov_txt        = :tipo";            $params[':tipo']      = $tipo; }
if ($art       !== '') { $where[] = "TRIM(v.cve_articulo)  = TRIM(:art)";       $params[':art']       = $art; }
if ($lote      !== '') { $where[] = "v.lote LIKE :lote";                         $params[':lote']      = "%$lote%"; }
if ($proveedor !== '') { $where[] = "TRIM(v.cve_proveedor) = TRIM(:proveedor)"; $params[':proveedor'] = $proveedor; }
if ($proyecto  !== '') { $where[] = "TRIM(v.cve_proyecto)  = TRIM(:proyecto)";  $params[':proyecto']  = $proyecto; }
...
if (isset($_GET['action'])) {
  ...
  if ($a === 'kpis') {
    ...
    header('Content-Type: application/json'); echo json_encode($k); exit;
  }
  ...
  if ($a === 'lotes_por_art') {
    if ($art === '') { header('Content-Type: application/json'); echo json_encode([]); exit; }
    $rows = db_all("
...
<div class="container-fluid">
  ...
  <div class="ap-card p-4 mb-3">
    ...
    <p class="text-secondary m-0">Análisis de movimientos por almacén, usuario, proyecto y proveedor. (Gráficas 250px)</p>
  </div>
    ...
    <div class="row g-2">
        ...
        <select name="art" id="sel-art" class="form-select">
          ...
          <?php foreach ($cat_art as $a): ?>
            <option value="<?=$a['cve_articulo']?>" <?=($art===$a['cve_articulo']?'selected':'')?>>
              <?=htmlspecialchars($a['desc_articulo'] !== '' ? $a['desc_articulo'] : $a['cve_articulo'])?> (<?=$a['cve_articulo']?>)
            </option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div class="col-12 col-md-3">
        ...
        <select name="lote" id="sel-lote" class="form-select">
          <option value="">(Todos)</option>
          <?php if ($lote !== ''): ?><option selected value="<?=htmlspecialchars($lote)?>"><?=htmlspecialchars($lote)?></option><?php endif; ?>
        </select>
      </div>
  ...
  <div class="row g-3 mb-3">
    <div class="col-6 col-md-3"><div class="ap-card p-3"><div class="text-secondary small">Total Movimientos</div><div class="h4 m-0" id="kpi-total">-</div></div></div>
    <div class="col-6 col-md-3"><div class="ap-card p-3"><div class="text-secondary small">Entradas</div><div class="h4 m-0" id="kpi-entradas">-</div></div></div>
    <div class="col-6 col-md-3"><div class="ap-card p-3"><div class="text-secondary small">Salidas</div><div class="h4 m-0" id="kpi-salidas">-</div></div></div>
    <div class="col-6 col-md-3"><div class="ap-card p-3"><div class="text-secondary small">Balance Neto</div><div class="h4 m-0" id="kpi-balance">-</div></div></div>
  </div>

  <!-- KPIs fila 2 -->
  <div class="row g-3 mb-4">
    <div class="col-6 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">Mov/Hora</div><div class="h4 m-0" id="kpi-movhora">-</div></div></div>
    <div class="col-6 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">% Entradas</div><div class="h4 m-0" id="kpi-pctent">-</div></div></div>
    <div class="col-6 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">% Salidas</div><div class="h4 m-0" id="kpi-pctsal">-</div></div></div>
    <div class="col-6 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">Días Activos</div><div class="h4 m-0" id="kpi-diasact">-</div></div></div>
    <div class="col-12 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">Top Operador</div><div class="h6 m-0" id="kpi-topoper">-</div><div class="text-secondary small"><small id="kpi-topoper-cnt"></small></div></div></div>
    <div class="col-12 col-md-2"><div class="ap-card p-3"><div class="text-secondary small">Top Almacén</div><div class="h6 m-0" id="kpi-topalm">-</div><div class="text-secondary small"><small id="kpi-topalm-cnt"></small></div></div></div>
  </div>
