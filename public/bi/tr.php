...
{
  global $COLLATE;
  return "CONVERT($expr USING utf8mb4) COLLATE {$COLLATE}";
}
...
switch ($dataset) {
  ...
  case 'proveedores':
    ...
    if ($hasEmpresaProv)
      $empresas_ds = db_all("SELECT DISTINCT empresa_id AS empresa FROM c_proveedores WHERE empresa_id IS NOT NULL AND empresa_id<>'' ORDER BY empresa_id");
    break;
  default:
    $empresas_ds = [];
}
...
if ($dataset === 'almacenes') {
  $cols = ['Empresa', 'Clave', 'Nombre', 'Direccion', 'CP', 'Telefono', 'Contacto', 'Correo', 'Activo'];
} elseif ($dataset === 'usuarios') {
  $cols = ['Empresa', 'Clave', 'Nombre', 'Correo', 'Celular', 'Rol', 'Estatus', 'Activo'];
} elseif ($dataset === 'proveedores') {
  $cols = ['Clave', 'Nombre', 'Pais', 'Estado', 'Ciudad', 'Telefono', 'Cliente', 'Activo'];
} // SOLO clientes
else {
  $cols = ['Clave', 'Descripcion', 'Grupo', 'Clasificacion', 'Tipo', 'Activo'];
}
...
if ($dataset === 'almacenes') {
  $title = 'Almacenes';
  if ($empresa_id !== '') {
    $where .= " AND empresa_id = :emp";
    $params[':emp'] = $empresa_id;
  }
  if ($activo === '1') {
    $where .= " AND ((COALESCE(Activo,'')+0)=1 OR " . COL('Activo') . " IN ('S','SI','Y','YES'))";
  }
  if ($activo === '0') {
    $where .= " AND NOT ((COALESCE(Activo,'')+0)=1 OR " . COL('Activo') . " IN ('S','SI','Y','YES'))";
  }
  if ($q) {
  ...
  $where = " WHERE 1=1 ";
  if ($empresa_id !== '') {
    $where .= " AND cu.empresa_id = :emp";
    $params[':emp'] = $empresa_id;
  }
  if ($activo === '1') {
    $where .= " AND ((COALESCE(cu.Activo,'')+0)=1 OR " . COL('cu.Activo') . " IN ('S','SI','Y','YES'))";
  }
  if ($activo === '0') {
    $where .= " AND NOT ((COALESCE(cu.Activo,'')+0)=1 OR " . COL('cu.Activo') . " IN ('S','SI','Y','YES'))";
  }
  if ($rol !== '') {
    $where .= " AND (UPPER(COALESCE(tr.rol, cu.perfil)) COLLATE {$COLLATE}) = (UPPER(CAST(:rol AS CHAR)) COLLATE {$COLLATE})";
    $params[':rol'] = $rol;
  }
  if ($q) {
  ...
  if ($alm_sesion !== '') {
    $where .= " AND " . COL('tua.cve_almac') . " = " . COL(':alm');
...
<?php if ($IFRAME): ?>
  ...
  <head>
    ...
    .card-azul {
      background: linear-gradient(135deg, #0F5AAD, #2A76D2);
    }
  ...
  <div class="container-fluid">
      ...
      <div class="filters-grid">
        ...
        <div class="col">
          ...
          <input type="text" class="form-control form-control-sm" name="q" value="<?= htmlspecialchars($q ?? '') ?>"
            placeholder="Clave, nombre, correo, ciudad...">
        </div>

        <div style="display:flex; gap:8px; align-items:end;">
          <div>
            <label class="form-label mb-1">Filas/pág.</label>
            <input type="number" class="form-control form-control-sm" name="per_page" value="<?= (int) $per_page ?>"
              min="10" step="5">
          </div>
          <div style="display:flex; gap:8px; margin-bottom:2px;">
            <button class="btn btn-sm btn-primary" type="submit">Aplicar</button>
            <a class="btn btn-sm btn-outline-secondary"
              href="?<?= $IFRAME ? 'iframe=1&' : '' ?>dataset=<?= urlencode($dataset) ?>">Limpiar</a>
        ...
        <thead class="table-light">
          <tr>
            <?php foreach ($cols as $c): ?>
              <th><?= htmlspecialchars($c) ?></th><?php endforeach; ?>
          </tr>
        </thead>
        <tbody>
          <?php if (!$rows): ?>
            <tr>
              <td colspan="<?= count($cols) ?>">Sin resultados.</td>
            </tr>
          <?php else:
            foreach ($rows as $r): ?>
              <tr>
    ...
    <div class="d-flex gap-2 align-items-center mt-2 flex-wrap">
      <a class="btn btn-sm btn-outline-secondary <?= $disablePrev ? 'disabled' : '' ?>" href="?<?= qs(['p' => 1]) ?>">«
        Primero</a>
      <a class="btn btn-sm btn-outline-secondary <?= $disablePrev ? 'disabled' : '' ?>" href="?<?= qs(['p' => $prev]) ?>">‹
        Anterior</a>
      <span class="text-muted">Página <strong><?= $page ?></strong> de <strong><?= $pages ?></strong> — Registros:
        <strong><?= number_format($total) ?></strong></span>
      <a class="btn btn-sm btn-outline-secondary <?= $disableNext ? 'disabled' : '' ?>"
        href="?<?= qs(['p' => $next]) ?>">Siguiente ›</a>
      <a class="btn btn-sm btn-outline-secondary <?= $disableNext ? 'disabled' : '' ?>"
        href="?<?= qs(['p' => $pages]) ?>">Último »</a>
    </div>
  </div>
