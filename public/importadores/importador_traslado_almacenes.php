...
.card-lite .value{ font-size:22px; font-weight:700 }
...
<div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
  ...
  <div class="d-flex gap-2">
    ...
    <a href="Layout_Traslado_entre_Almacenes1.csv" class="btn btn-outline-primary" download>
...
<div class="row g-3 mb-4">
  ...
  <div class="col-md-3">
    <div class="card-lite p-3">
      ...
      <div class="value" id="kStatus">Borrador</div>
    </div>
  </div>
  <div class="col-md-3">
...
<div class="card-lite p-3">
  ...
  <div class="text-muted mb-2" id="previewInfo">Selecciona un archivo</div>

  <div class="table-responsive">
    <table class="table table-sm table-bordered">
      <thead id="previewHead"><tr><th>—</th></tr></thead>
      <tbody id="previewBody"><tr><td>—</td></tr></tbody>
    </table>
  </div>
</div>
...
let currentRunId = 0;     // solo existe después de Guardar
let currentFolio = '';    // folio negocio
let lastFile = null;
...
async function parseCsv(file){
  ...
  const rows = lines.slice(1).map(l=>l.split(delim).map(v=>clean(v)));
...
async function renderPreview(file){
  ...
  if(!headers.length){
    previewHead.innerHTML = '<tr><th>—</th></tr>';
    previewBody.innerHTML = '<tr><td>—</td></tr>';
    previewInfo.innerText = `Archivo: ${file.name} · Sin filas detectadas`;
    return;
  }
  ...
  previewBody.innerHTML = previewRows.map(r=>{
    return '<tr>' + headers.map((_,i)=>`<td>${r[i] ?? ''}</td>`).join('') + '</tr>';
  }).join('') || `<tr><td colspan="${headers.length}">—</td></tr>`;

  previewInfo.innerText = `Archivo: ${file.name} · Filas detectadas: ${rows.length}`;
}

/**
 * VALIDACIÓN LOCAL (NO BD)
 * Objetivo: llenar cards y detectar problemas básicos de layout/campos vacíos
 * Layout esperado:
 * BL_ORIGEN, LP_O_PRODUCTO, LOTE_SERIE, CANTIDAD, ZRD_BL (o ZONA_RECIBO_DESTINO)
 */
function localValidate(headers, rows){
  const H = headers.map(h=>clean(h).toUpperCase());
  const idx = (name)=>H.indexOf(name);

  const iBL = idx('BL_ORIGEN');
  const iLP = idx('LP_O_PRODUCTO');
  const iZ1 = idx('ZRD_BL');
  const iZ2 = idx('ZONA_RECIBO_DESTINO');
  const iZ  = (iZ1 !== -1) ? iZ1 : iZ2;
  ...
  // Si layout inválido, no iterar filas
  if(errors > 0){
    return {
      total: rows.length,
      err: errors,
      status: 'Layout inválido'
    };
  }

  // Validación básica por fila: campos obligatorios no vacíos
  rows.forEach(r=>{
    if(!clean(r[iBL])) errors++;
    if(!clean(r[iLP])) errors++;
    if(!clean(r[iZ]))  errors++;
  });

  return {
    total: rows.length,
    err: errors,
    status: errors>0 ? 'Validación previa con errores' : 'Validación previa OK'
  };
}
...
csvFile.addEventListener('change', async ()=>{
  ...
  // permite seleccionar el mismo archivo sin recargar
  csvFile.value = '';
});

// Vista previa | Validar
// Regla: NO debe crear corrida.
// - Si no hay corrida guardada => validación LOCAL (cards llenas, 0 inserts)
// - Si ya hay corrida => revalida en backend con run_id fijo
btnPrev.addEventListener('click', async ()=>{
  if(!lastFile) return;

  const parsed = await parseCsv(lastFile);
  await renderPreview(lastFile);

  if(!currentRunId){
    const v = localValidate(parsed.headers, parsed.rows);

    kTotal.innerText  = v.total;
    kErr.innerText    = v.err;
    kStatus.innerText = v.status;

    lastValidationOK = (v.err === 0);

    // Procesar NO permitido sin Guardar (gobierno)
    btnProc.disabled = true;
    return;
  }

  // Ya hay corrida: valida con backend SIN crear otra
  ...
  if(!j.ok) return alert(j.error || 'Error al validar');
...
// Guardar (crea corrida) + mensaje friendly con FOLIO (no ID)
btnSave.addEventListener('click', async ()=>{
  ...
  if(!currentRunId) return alert('Error: no se pudo obtener el ID de corrida');

  // Folio negocio: contrato real via GET
  const rg = await fetch(`${API_GET}?run_id=${encodeURIComponent(currentRunId)}`);
  const gj = await rg.json();
  if(!gj.ok || !gj.run || !gj.run.folio_importacion){
    return alert('Error: no se pudo obtener folio de importación');
  }
  currentFolio = gj.run.folio_importacion;

  // Cards (resultado de validate)
  kTotal.innerText = j.totales.total_lineas;
  kErr.innerText   = j.totales.total_err;
  ...
  alert(`Guardado exitoso · Folio ${currentFolio}`);
});
...
btnProc.addEventListener('click', async ()=>{
  ...
  const j = await r.json();
  if(!j.ok) return alert(j.error || 'Error al procesar');
