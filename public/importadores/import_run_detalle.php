...
(function(){
  ...
  // UI en /public/importadores/ ; APIs en /public/api/importadores/
  ...
  // ✅ Soporta alias: busca en orden de prioridad
  function getDataField(row, keys){
    if(!row || !row.data) return '-';
    const arr = Array.isArray(keys) ? keys : [keys];
    for(const k of arr){
      if(row.data[k] !== undefined && row.data[k] !== null && row.data[k] !== '') return row.data[k];
    }
    return '-';
  }
  ...
  async function load(){
    ...
    tb.innerHTML = rows.map(rw=>{
      ...
      // ✅ Aquí está la corrección: ZRD_BL puede venir como ZONA_RECIBO_DESTINO
      const zrd  = getDataField(rw, ['ZRD_BL', 'ZONA_RECIBO_DESTINO']);
