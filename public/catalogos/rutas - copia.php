...
<style>
  ...
  .dt-container { overflow: auto; max-height: calc(100vh - 320px); border: 1px solid #e6e6e6; border-radius: 12px; }
...
<script>
  ...
  // Bootstrap 5 a veces no queda cargado vía _menu_global_end.php en esta instancia.
  // Blindamos: si no existe `bootstrap.Modal`, cargamos bundle desde CDN y además dejamos fallback.
  let mdl = null;
  ...
  function ensureBootstrap(cb){
    ...
    s.onload = cb;
    s.onerror = cb; // en caso de bloqueo, seguimos con fallback
  ...
  function showModal(){
    const m = getModal();
    if (m) return m.show();
    // fallback para Bootstrap 4/jQuery modal
  ...
  async function load(){
    ...
    // Render seguro: DataTables (jQuery) si existe, si no, HTML simple
    ...
    // Fallback sin DataTables: pintamos filas
    const tb = document.querySelector('#tbl tbody');
    tb.innerHTML = rows.map(r => {
      const tds = cols.map(c => `<td>${(r[c.data] ?? '')}</td>`).join('');
      return `<tr>${tds}</tr>`;
    }).join('');
    return;
  }
