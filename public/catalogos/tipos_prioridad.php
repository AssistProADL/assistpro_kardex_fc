...
<div class="ap-container">
  <div class="ap-title"><i class="fa fa-bolt"></i> Catálogo · Tipos de Prioridad</div>
  ...
  <div class="ap-cards">
    ...
    <div class="ap-card" onclick="abrirImport();">
      ...
      <div class="k"><span class="ap-chip">UPSERT por Clave</span></div>
    </div>
  </div>
  ...
  <div class="ap-toolbar">
    <div class="ap-search">
      ...
      <input id="q" placeholder="Buscar por Clave, Descripción, Prioridad o Status…" onkeydown="if(event.key==='Enter')buscar()">
...
<div class="ap-modal" id="mdl">
  <div class="ap-modal-content">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
      <h3 style="margin:0"><i class="fa fa-bolt"></i> Tipo de Prioridad</h3>
      <div class="ap-chip"><span style="color:#dc3545;font-weight:700">*</span> Obligatorios: <b>Clave</b>, <b>Descripcion</b></div>
    </div>

    <input type="hidden" id="ID_Tipoprioridad">

    <div class="ap-form" style="margin-top:10px">
      <div class="ap-field">
        <div class="ap-label">Clave *</div>
        <div class="ap-input"><i class="fa fa-hashtag"></i><input id="Clave" placeholder="PRI-001"></div>
        <div class="ap-error" id="err_clave">Clave obligatoria.</div>
      </div>

      <div class="ap-field">
        <div class="ap-label">Descripción *</div>
        <div class="ap-input"><i class="fa fa-align-left"></i><input id="Descripcion" placeholder="Urgente / Alta / Media / Baja"></div>
        <div class="ap-error" id="err_desc">Descripcion obligatoria.</div>
      </div>
      ...
      <div class="ap-field">
        <div class="ap-label">Prioridad (num)</div>
        <div class="ap-input"><i class="fa fa-sort-numeric-up"></i><input id="Prioridad" placeholder="1,2,3..." oninput="this.value=this.value.replace(/[^0-9]/g,'')"></div>
      </div>

      <div class="ap-field">
        <div class="ap-label">Status (legacy)</div>
        <div class="ap-input"><i class="fa fa-flag"></i>
          <select id="Status">
...
<div class="ap-modal" id="mdlImport">
  <div class="ap-modal-content">
    <h3><i class="fa fa-upload"></i> Importar Tipos de Prioridad</h3>
    <div class="ap-chip">Layout FULL con UPSERT por <b>Clave</b>. Previsualiza antes de importar.</div>
...
<script>
const API = '../api/tipos_prioridad.php';
const KPI = '../api/tipos_prioridad_kpi.php';
...
function reqDot(r){
  const ok = !!(String(r.Clave||'').trim()!=='' && String(r.Descripcion||'').trim()!=='');
  ...
  fetch(url).then(r=>r.json()).then(resp=>{
    ...
    rows.forEach(r=>{
      ...
      <tr>
        <td class="ap-actions">
          ${verInactivos
            ? `<i class="fa fa-undo" title="Recuperar" onclick="recuperar(${r.ID_Tipoprioridad})"></i>`
            : `<i class="fa fa-edit" title="Editar" onclick="editar(${r.ID_Tipoprioridad})"></i>
               <i class="fa fa-trash" title="Inactivar" onclick="eliminar(${r.ID_Tipoprioridad})"></i>`}
        </td>
        <td>${reqDot(r)}</td>
        <td>${r.ID_Tipoprioridad||''}</td>
        <td>${r.Clave||''}</td>
        <td>${r.Descripcion||''}</td>
        <td>${(r.Prioridad===null||r.Prioridad===undefined)?'':r.Prioridad}</td>
        <td>${r.Status||''}</td>
...
function hideErrors(){ err_clave.style.display='none'; err_desc.style.display='none'; }
function validar(){
  hideErrors();
  let ok=true;
...
function guardar(){
  ...
  fd.append('action', ID_Tipoprioridad.value ? 'update' : 'create');
...
function eliminar(id){
  if(!confirm('¿Inactivar tipo de prioridad?')) return;
