...
<head>
    ...
    <style>
        ...
        .badge-soft{ background:#f2f4f8; border:1px solid #e4e7ec; color:#344054; }
        .btn-xxs{ padding:.15rem .4rem; font-size:.75rem; }
...
function apiGet(params){
    ...
    return fetch(`${API}?${qs}`, { credentials:'same-origin' }).then(r=>r.json());
}
...
function loadTabla(){
    ...
    dt = $('#tblReglas').DataTable({
        ...
        ajax: function(data, cb){
            apiGet({action:'get', id: promoId}).then(res=>{
                const rows = (res && res.ok && res.data && Array.isArray(res.data.rules)) ? res.data.rules : [];
                cb({data: rows});
            }).catch(()=>{
                cb({data: []});
            });
        ...
        columns: [
            ...
            { data:'trigger_tipo', defaultContent:'' },
            { data:'threshold_monto', defaultContent:'' },
            { data:'threshold_qty', defaultContent:'' },
            { data:'acumula', defaultContent:'' },
            { data:'acumula_por', defaultContent:'' },
            ...
            { data:null, orderable:false, render: function(r){
                ...
                return `
                    <div class="d-flex flex-wrap gap-1">
                        <button class="btn btn-outline-primary btn-xxs" onclick='editar(${JSON.stringify(r)})'>Editar</button>
                        <a class="btn btn-outline-secondary btn-xxs" href="promocion_beneficios.php?${q.toString()}">Rewards</a>
                        <a class="btn btn-outline-secondary btn-xxs" href="promocion_scope.php?promo_id=${encodeURIComponent(promoId)}${almacenId?`&almacen_id=${encodeURIComponent(almacenId)}`:''}">Scope</a>
                        <button class="btn btn-outline-danger btn-xxs" onclick="eliminar(${r.id_rule})">Eliminar</button>
                    </div>
                `;
            }}
        ],
...
window.eliminar = function(id_rule){
    ...
    apiPost('rule_del', {id_rule: id_rule}).then(res=>{
        if(res && res.ok){
            loadTabla();
        }else{
