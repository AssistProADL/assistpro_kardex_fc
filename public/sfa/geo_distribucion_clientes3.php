...
async function cargarClientes(){
  const emp = document.getElementById('f_empresa').value;
  const rutaRaw = (document.getElementById('f_ruta').value || '').trim();
  // Normaliza el valor 'todas' para compatibilidad con el API (V1 usa 'all')
  const ruta = (!rutaRaw || rutaRaw === '0' || rutaRaw.toLowerCase() === 'all') ? 'all' : rutaRaw;
