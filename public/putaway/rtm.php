...
<head>
  ...
  <style>
    ...
    /* El marco corporativo ya define layout; aquí evitamos “doble padding” */
    body{ background:#f6f8fb; margin:0; padding:0; }

    /* Contenedor “full” para no desfasarse dentro del frame */
    .ap-wrap{
      width:100%;
      max-width:100%;
      padding: 12px 16px 18px 16px; /* ligero para que no pegue al borde */
      box-sizing:border-box;
    }

    .ap-title{ color:var(--ap-blue2); font-weight:800; letter-spacing:.2px; margin:0; }
    .ap-sub{ color:#5b6b7a; font-size:12px; margin-top:2px; }
    ...
    /* Tabla compacta y estable dentro de frame */
    .ap-table-wrap{
      height: calc(100vh - 260px); /* ajusta según tu header corporativo */
      min-height: 320px;
      overflow:auto;
      border-radius:12px;
    }

    table.ap-table{ font-size:10px; white-space:nowrap; margin:0; }
    table.ap-table th{ position:sticky; top:0; background:#f1f5ff; z-index:2; }
    table.ap-table td, table.ap-table th{ vertical-align:middle; }
    ...
    .ap-spinner{ display:none; align-items:center; gap:6px; }
    .ap-muted{ color:#6c7a89; }

    /* Evitar que el contenido “se meta” debajo de barras/headers del frame */
    .ap-head-row{ gap:10px; flex-wrap:wrap; }
    .ap-head-actions{ display:flex; gap:10px; align-items:center; }
  </style>
</head>
...
<body>
  <div class="ap-wrap">
    ...
    <div class="d-flex align-items-start justify-content-between ap-head-row mb-2">
      <div>
        <h2 class="ap-title"><i class="fa-solid fa-dolly me-2"></i>Ready To Move</h2>
      ...
      <div class="ap-head-actions">
        <div class="ap-spinner" id="spinner">
          ...
          <span class="ap-muted small">Cargando…</span>
        </div>
    ...
    <div class="ap-card p-2">
      <div class="ap-table-wrap">
        <table class="table table-sm table-bordered table-hover text-center ap-table mb-0" id="tbl">
          <thead>
            <tr>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <div>
            <h6 class="modal-title m-0">
              <i class="fa-solid fa-list-check me-2"></i>Detalle de folio <span id="dFolio"></span>
            </h6>
        ...
        <div class="modal-body">
          <div class="table-responsive">
            <table class="table table-sm table-bordered table-hover text-center ap-table" id="tblDetail">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Artículo</th>
  ...
  <script>
    ...
    async function loadRTM(){
      ...
      const rows = (j.data||[]).filter(x=>{
        if(!q) return true;
        return (String(x.folio||'')+' '+String(x.proveedor||'')+' '+String(x.protocolo||'')+' '+String(x.tipo||''))
