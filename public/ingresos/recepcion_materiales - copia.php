...
// API global de filtros AssistPro
...
$(document).ready(function () {
    ...
    $('#cboArticulo').on('change', function () {
        var val = $('#cboArticulo').val();
        var txt = $('#cboArticulo option:selected').text();
        $('#txtDescArticulo').val(txt);

        // UOM desde filtros_assistpro si existe catálogo de productos
        $('#cboUM').empty().append('<option value="">UM</option>');
        if (filtrosAssistPro && Array.isArray(filtrosAssistPro.productos)) {
            var prod = filtrosAssistPro.productos.find(function (p) {
                return (p.cve_articulo || '').toString() === val;
            });
            if (prod) {
                var u = prod.cve_umed || prod.um || '';
                if (u) {
                    $('#cboUM').append(
                        $('<option>', {
                            value: u,
        ...
        // (Si no hay datos en API, se queda la opción manual)
    });
...
// ====== CARGA INICIAL DESDE filtros_assistpro.php ======
    ...
    $.ajax({
        ...
        success: function (resp) {
            if (!resp || resp.ok === false) {
                console.warn('Error en filtros_assistpro:', resp && resp.error ? resp.error : '');
                return;
            }
            filtrosAssistPro = resp;
        ...
        error: function () {
            console.warn('No se pudo cargar filtros_assistpro.php');
        }
    });
}
...
function cargarEmpresasDesdeFiltros() {
    ...
    if (filtrosAssistPro.empresas.length === 1) {
        var r0 = filtrosAssistPro.empresas[0];
        $cbo.val(r0.empresa_id || r0.cve_cia || r0.clave_empresa || '');
    }
}
...
function cargarAlmacenesDesdeFiltros() {
    ...
    filtrosAssistPro.almacenes.forEach(function (row) {
        var cve = row.cve_almac || row.Cve_Almac || row.clave_almacen || '';
        var des = row.des_almac || row.nombre || '';
        var txt = '[' + (cve || '') + '] ' + (des || '');
        $cbo.append(
            $('<option>', {
                value: cve,
...
function obtenerUbicacionesFiltradasPorAlmacen() {
    if (!filtrosAssistPro) return [];
    var ubic = filtrosAssistPro.ubicaciones || filtrosAssistPro.zonas || filtrosAssistPro.bls || [];
    if (!Array.isArray(ubic)) return [];
    var almSel = $('#cboAlmacen').val();
    return ubic.filter(function (u) {
        var ua = (u.cve_almac || u.Cve_Almac || u.CveAlmac || '').toString();
        if (!almSel) return true;
        return ua === almSel;
    });
}
...
function cargarZonasRecepcionDesdeFiltros() {
    ...
    var ubic = obtenerUbicacionesFiltradasPorAlmacen();
    if (!ubic.length) return;

    ubic.forEach(function (row) {
        var id  = row.id || row.idy_ubica || '';
        var bl  = row.CodigoCSD || row.codigocsd || row.codigo_bl || row.bl || '';
        var nom = row.nombre || row.descripcion || '';
        var txt = (bl ? (bl + ' - ') : '') + (nom || '');
        $cbo.append(
            $('<option>', {
                value: id || bl,
...
function cargarZonasDestinoDesdeFiltros() {
...
function cargarBLDestinoDesdeFiltros() {
    var $cbo = $('#cboBLDestino');
    $cbo.empty().append('<option value="">Seleccione BL destino</option>');
    var zonaDest = $('#cboZonaDestino').val();
    if (!filtrosAssistPro) return;

    var ubic = filtrosAssistPro.ubicaciones || filtrosAssistPro.bls || [];
    if (!Array.isArray(ubic)) return;

    ubic.forEach(function (row) {
        var id  = row.id || row.idy_ubica || '';
        var bl  = row.CodigoCSD || row.codigocsd || row.codigo_bl || row.bl || '';
        if (!zonaDest || id == zonaDest) {
            $cbo.append(
                $('<option>', {
                    value: id || bl,
                    text: bl || id
...
function cargarArticulosDesdeFiltros() {
    ...
    prods.forEach(function (p) {
        var cve = p.cve_articulo || p.Cve_Articulo || '';
        var des = p.des_articulo || p.Descripcion || '';
...
// ====== CARGAS ADICIONALES (OCs, guardar) CON API recepcion_materiales.php ======
function cargarOCs() {
