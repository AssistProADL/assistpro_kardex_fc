<?php
/* kardex.php (fast)
 * Carga rápida: no consulta BD hasta que el usuario presione "Filtrar".
 * - Evita COUNT(*) (costoso). Para paginación usa técnica de limit+1.
 * - Filtros básicos y CSV.
 */
...
$rows = []; $has_prev = false; $has_next = false;
if ($run) {
  $st = $pdo->prepare("
  ...
  $st->bindValue(':lim', (int)($limit+1), PDO::PARAM_INT); // pedimos uno extra
...
<body>
<div class="container">
  <h1>Kardex Bidireccional — ETL (carga rápida)</h1>
