<?php
/* kardex_etl_v2.php — Endpoint ETL compatible con kardex_actual.php */
error_reporting(0);
header('Content-Type: application/json; charset=utf-8');

$REQ = array();
foreach (['_GET','_POST'] as $__super) { foreach ($$__super as $k=>$v) { $REQ[$k]=$v; } }
$action = isset($REQ['action']) ? $REQ['action'] : (isset($REQ['accion']) ? $REQ['accion'] : '');
$debug  = isset($REQ['debug']) ? $REQ['debug'] : 0;
...
if (!$__dbLoaded) {
    try {
        ...
        echo json_encode(['success'=>false,'msg'=>'Error de conexión','error'=>$e->getMessage()]); exit;
    }
}
...
$supported = ['enter-view','getLotesArticulosKardex','loadDetalleCajas','help','ping'];

if ($action === 'help' || $action === '') {
    echo json_encode(['success'=>true,'supported_actions'=>$supported,'hint_examples'=>[
        'GET enter-view'=>'kardex_etl_v2.php?action=enter-view&id_user=1',
        'POST getLotesArticulosKardex'=>'action=getLotesArticulosKardex&empresa_id=1&cve_articulo=SKU123',
        'POST loadDetalleCajas'=>'action=loadDetalleCajas&empresa_id=1&almacen=1&lp=LP001&page=1&rows=50&sidx=fecha_hora&sord=DESC'
    ],'received'=>$debug?$REQ:null]); exit;
}
if ($action==='ping'){ echo json_encode(['success'=>true,'pong'=>true,'received'=>$debug?$REQ:null]); exit; }
...
if ($action==='enter-view'){
    ...
    foreach($almacenes as $r){ $v=htmlspecialchars($r['id']??$r['clave']??''); $t=htmlspecialchars(($r['clave']??'').' - '.($r['des_almac']??'')); $options.="<option value='".$v."'>".$t."</option>"; }
    echo json_encode(['success'=>true,'almacen'=>$options,'debug'=>$debug?$REQ:null]); exit;
}

if ($action==='getLotesArticulosKardex'){
    $art=isset($REQ['cve_articulo'])?trim($REQ['cve_articulo']):'';
    $emp=isset($REQ['empresa_id'])?trim($REQ['empresa_id']):'';
    if($art===''){ echo json_encode(['success'=>true,'lotes'=>"<option value=''>Seleccione Lote | Serie (0)</option>",'debug'=>$debug?$REQ:null]); exit; }
    if(!hasView($pdo,'v_kardex_doble_partida')){ echo json_encode(['success'=>false,'lotes'=>"<option value=''>No existe vista v_kardex_doble_partida</option>"]); exit; }
    ...
    echo json_encode(['success'=>true,'lotes'=>$options,'debug'=>$debug?$REQ:null]); exit;
}

if ($action==='loadDetalleCajas'){
    $page=(int)($REQ['page']??1); $limit=(int)($REQ['rows']??50);
    $sidx=$REQ['sidx']??'fecha_hora'; $sord=$REQ['sord']??'DESC';
    $almacen=$REQ['almacen']??null; $lp=$REQ['lp']??null; $emp=$REQ['empresa_id']??null;
    ...
    if($has_ec && $has_u && $has_ch && $has_al){
        ...
        $resp=(object)['page'=>$page,'total'=>$total_pages,'records'=>$total,'rows'=>[]];
...
echo json_encode(['success'=>false,'msg'=>'Acción no soportada en ETL','supported'=>$supported,'received'=>$debug?$REQ:null]); exit;
?>