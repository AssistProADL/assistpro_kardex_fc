...
if ($op) {
  ...
  try {
    ...
    if ($op === 'kpi_compuestos') {
      ...
      echo json_encode(['ok'=>true,'total'=>$n]); exit;
    }
    // KPI: total artículos
    ...
    if ($op === 'get_componentes') {
      $padre = trim($_POST['padre'] ?? $_GET['padre'] ?? '');
      if ($padre==='') throw new Exception('Artículo compuesto requerido');
      $sql = "
        SELECT
        ...
        LEFT JOIN c_articulo a ON a.cve_articulo = c.Cve_ArtComponente
      ...
      <html><head><meta charset="utf-8">
        ...
        <div class="hdr">
          ...
          <div>
            ...
            <div class="muted"><?php echo htmlspecialchars($producto['des_articulo'] ?? ''); ?> | UMed: <?php echo htmlspecialchars($producto['unidadMedida'] ?? ''); ?></div>
    ...
    if ($op === 'import_bom') {
      ...
      if (!$fh) throw new Exception('No fue posible leer el archivo.');

      $peek = fgetcsv($fh);
      if ($peek === false) throw new Exception('Archivo vacío.');
      $isHeader = false;
      if (count($peek) >= 2) {
        $a0 = mb_strtolower(trim($peek[0]??'')); $a1 = mb_strtolower(trim($peek[1]??''));
        if (!preg_match('/[0-9]/',$a0) && !preg_match('/[0-9]/',$a1)) $isHeader = true;
      }
      if (!$isHeader) { rewind($fh); }
      ...
      while (($row = fgetcsv($fh)) !== false) {
        ...
        if ($padre==='' || $hijo==='') { $errors[]="L{$line}: producto compuesto / componente vacío"; continue; }

        $cantRaw = str_replace([' ', ','], ['', '.'], $cantRaw);
        $cantidad = is_numeric($cantRaw) ? (float)$cantRaw : null;
        if ($cantidad === null) { $errors[]="L{$line}: cantidad inválida"; continue; }
      ...
      if (empty($rowsByParent)) {
        echo json_encode(['ok'=>false,'msg'=>'No hay filas válidas para importar.','errors'=>$errors]); exit;
      }
      ...
      if (!empty($todos)) {
        ...
        foreach ($rows as $r) $existentes[$r['cve_articulo']] = true;
      }

      foreach (array_keys($parentsSet) as $p) {
        if (!isset($existentes[$p])) $errors[] = "Padre no existe en c_articulo: {$p}";
      }
      foreach (array_keys($componentsSet) as $c) {
        if (!isset($existentes[$c])) $errors[] = "Componente no existe en c_articulo: {$c}";
      }
...
<style>
  ...
  .kpi-card{background:#f8fbff;border:1px solid #e6eef8;border-radius:.75rem;}
  .kpi-title{font-size:10px;color:#6c757d;margin-bottom:2px;}
  .kpi-value{font-size:22px;font-weight:700;color:#0F5AAD;line-height:1;}
    ...
    <div class="card-body">
      ...
      <div class="table-responsive mb-3">
        <table id="tblResultados" class="table table-sm table-striped table-hover w-100">
          <thead><tr><th class="dt-center">Acciones</th><th>Clave</th><th>Descripción</th><th>UMed</th></tr></thead>
          <tbody></tbody>
        </table>
      </div>
      ...
      <div class="table-responsive">
        <table id="tblComponentes" class="table table-sm table-striped table-hover w-100">
          <thead><tr><th>Componente</th><th>Descripción</th><th class="dt-right">Cantidad</th><th>Unidad</th><th>Etapa</th></tr></thead>
          <tbody></tbody>
        </table>
      ...
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        ...
        <div class="btn-group btn-group-sm">
          <a class="btn btn-outline-secondary" href="bom.php?op=download_layout"><i class="bi bi-filetype-csv"></i> Descargar layout</a>
        </div>
      </div>
      <div class="row g-2 align-items-center mt-1">
        <div class="col-md-6">
          ...
          <div class="muted mt-1">
            Formato: <strong>A</strong>=producto_compuesto, <strong>B</strong>=componente, <strong>C</strong>=cantidad.  
            La unidad de medida se toma de <code>c_articulo.unidadMedida</code> del componente.
          </div>
        </div>
        <div class="col-md-3">
          <div class="btn-group w-100">
            <button id="btnPreview" class="btn btn-outline-primary btn-sm"><i class="bi bi-eye"></i> Previsualizar</button>
            <button id="btnImport" class="btn btn-success btn-sm" disabled><i class="bi bi-cloud-upload"></i> Importar</button>
          </div>
        </div>
      </div>
      ...
      <div id="impPanel" class="mt-2 d-none">
        ...
        <div class="table-responsive">
          <table id="tblPreview" class="table table-sm table-striped table-hover w-100">
            <thead><tr><th>#</th><th>Producto compuesto</th><th>Componente</th><th>Cantidad</th><th>Estado</th></tr></thead>
            <tbody></tbody>
          </table>
        </div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div>
          ...
          <pre id="m_errs" class="mt-1" style="font-size:10px; white-space:pre-wrap; max-height:160px; overflow:auto;"></pre>
        </div>
