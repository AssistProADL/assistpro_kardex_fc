  ...
  if ($op === 'guardar_ot') {
    ...
    try {
      ...
      $sqlH = "INSERT INTO t_ordenprod
        ...
        VALUES
        (:folio, :folioimp, :alm, :art, :cant, :fecha, NOW(),
    ...
    <div class="container-fluid mt-2">
        ...
        <div class="card mb-2">
            ...
            <div class="card-body">
                ...
                <div class="row g-2 mb-2">
                    <div class="col-md-3">
                        <label class="form-label mb-0">BL Destino (PT)</label>
                        <select id="selBLDestino" class="form-select form-select-sm"></select>
                        <div class="subtext">Default sugerido: <b>CEKANBAN02</b>. Se usa como BL destino del PT.</div>
                    </div>
                </div>
    ...
    <script>
      ...
      ${msg}
            ...
            function loadZonas() {
                ...
                const $bd = $('#selBLDestino').empty().append('<option value="">Seleccione BL destino</option>');
                if (!almacenp) return;
            ...
            function loadBLs() {
                ...
                $.getJSON('orden_produccion.php', { op: 'bl_by_zona', zona }, function (r) {
                    ...
                    (r.data || []).forEach(x => {
                        const v = (x.bl || '').replaceAll('"', '&quot;');
                        const o = `<option value="${v}">${x.bl}</option>`;
                        $b.append(o);
                        if ($d.length) $d.append(o);
                    });
                    // Defaults: origen = primer BL; destino = CEKANBAN02 si existe, si no primer BL
                    if ((r.data || []).length) {
                        $b.val(r.data[0].bl);
                        if ($d.length) {
                            const hasDefault = $d.find('option[value="CEKANBAN02"]').length > 0;
                            $d.val(hasDefault ? 'CEKANBAN02' : r.data[0].bl);
                        }
                    }
                });
            }
            ...
            $('#btnGuardarOT').on('click', function () {
                ...
                const empresa_id = parseInt($('#selEmpresa').val() || '0', 10);
                const almacen_id = parseInt($('#selAlmacen').val() || '0', 10);
                const zona_id = parseInt($('#selZona').val() || '0', 10);
                const bl_origen = ($('#selBLManu').val() || '').trim();
                const bl_destino = ($('#selBLDestino').val() || '').trim();
                const usuario = ($('#selUsuario').val() || '').trim();
                const cant = parseFloat($('#txtCantidadProducir').val() || '0');

                if (!folio) { toast('No hay folio de OT.', 'warning'); return; }
                if (!(empresa_id > 0) || !(almacen_id > 0) || !(zona_id > 0)) { toast('Falta empresa / almacén / zona.', 'warning'); return; }
                if (!bl_origen || !bl_destino) { toast('Falta BL origen o BL destino.', 'warning'); return; }
                if (!usuario) { toast('Selecciona el usuario que registra.', 'warning'); return; }
