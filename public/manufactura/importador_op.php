...
// Nota: En esta versión NO se afecta inventario ni kardex. Solo preview + validación.
// Fase 2 queda como stub para conectar a t_ordenprod/td_ordenprod y movimientos.
...
if ($action === 'process_phase2') {
  // Stub: aquí luego conectamos a lógica real (consolidación, afectación existencias, t_cardex, etc.)
  ap_send_json([
    'ok'=>0,
    'error'=>'Fase 2 aún no habilitada. Esta pantalla solo prepara preview y validaciones.'
  ], 200);
}
...
<div class="container-fluid py-3">
  <div class="d-flex align-items-start justify-content-between flex-wrap gap-2">
    <div>
      ...
      <div class="ap-sub">Previsualización + validaciones. (Afectación real se habilita en Fase 2 con consolidación y kardex.)</div>
    </div>
    <div>
      ...
      <div class="card ap-card-kpi">
        <div class="card-body py-2">
          ...
          <div class="m-0 ap-font-10">
            En esta fase no se actualizan <b>t_ordenprod / td_ordenprod</b> ni existencias ni <b>t_cardex</b>.
...
<script>
  ...
  $('#btnPhase2').on('click', async () => {
    ...
    // Stub
    try {
      const res = await fetch('?action=process_phase2', { method: 'POST' });
      const js = await res.json();
      if (!js.ok) {
        $('#mdlMsgErr').text(js.error || 'Fase 2 no disponible.');
        return;
      }
      $('#mdlMsgOk').text('Procesado.');
    } catch (e) {
      console.error(e);
