<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        if (!Schema::hasTable('v_cajamixta')) {
            Schema::create('v_cajamixta', function (Blueprint $table) {
                $table->integer('Cve_CajaMix');
                $table->string('Fol_Folio')->nullable();
                $table->integer('Sufijo')->nullable();
                $table->integer('NCaja')->nullable();
                $table->integer('Cve_TipoCaja')->nullable();
                $table->string('Abierta')->nullable();
                $table->string('Cve_Articulo')->nullable();
                $table->string('Cantidad')->nullable();
                $table->string('Cve_lote')->nullable();
                $table->tinyInteger('Activo')->default(1)->comment('Estado activo/inactivo');
                $table->timestamps(); // created_at y updated_at
            });
            
            // Set table engine, charset and collation (from database defaults)
            DB::statement("ALTER TABLE `v_cajamixta` ENGINE = InnoDB, CHARSET = utf8mb4, COLLATE = utf8mb4_unicode_ci");
        }
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('v_cajamixta');
    }
};