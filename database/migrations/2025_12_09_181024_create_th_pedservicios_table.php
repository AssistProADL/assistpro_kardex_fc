<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        if (!Schema::hasTable('th_pedservicios')) {
            Schema::create('th_pedservicios', function (Blueprint $table) {
                $table->integer('id_pedido');
                $table->string('Fol_Folio');
                $table->integer('Cve_Almac');
                $table->string('Cve_Usuario')->nullable();
                $table->date('Fec_Pedido')->nullable();
                $table->string('Cve_clte')->nullable();
                $table->string('Cve_CteProv')->nullable();
                $table->date('Fec_Inicio')->nullable();
                $table->date('Fec_Fin')->nullable();
                $table->string('Status')->nullable();
                $table->string('Observaciones')->nullable();
                $table->integer('Activo')->nullable();
                $table->integer('Forma_Pago')->nullable();
                $table->string('Docto_Ref')->nullable();
                $table->string('Docto_Ped')->nullable();
                $table->timestamps(); // created_at y updated_at
            });
            
            // Set table engine, charset and collation (from database defaults)
            DB::statement("ALTER TABLE `th_pedservicios` ENGINE = InnoDB, CHARSET = utf8mb4, COLLATE = utf8mb4_unicode_ci");
        }
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('th_pedservicios');
    }
};