<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        if (!Schema::hasTable('solicitudservicio')) {
            Schema::create('solicitudservicio', function (Blueprint $table) {
                $table->integer('id');
                $table->string('FolServicio')->nullable();
                $table->string('CodCliente')->nullable();
                $table->string('DescripcionServicio')->nullable();
                $table->integer('Prioridad')->nullable();
                $table->timestamp('Fecha')->nullable();
                $table->timestamp('FechaServicio')->nullable();
                $table->boolean('Cancelado')->nullable();
                $table->string('Observaciones')->nullable();
                $table->string('IdEmpresa')->nullable();
                $table->tinyInteger('Activo')->default(1)->comment('Estado activo/inactivo');
                $table->timestamps(); // created_at y updated_at
            });
            
            // Set table engine, charset and collation (from database defaults)
            DB::statement("ALTER TABLE `solicitudservicio` ENGINE = InnoDB, CHARSET = utf8mb4, COLLATE = utf8mb4_unicode_ci");
        }
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('solicitudservicio');
    }
};